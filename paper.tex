...
We use the following notations and assumptions to describe the LTC algorithm.
...
\begin{equation*}
\forall k \in \mathbb{N}, \  \exists ! j \in \mathbb{N} \  \xi_k = t_j
\end{equation*}
That is, transmission times coincide with reception times. 
We define the \emph{centered received points} as follows:
\begin{equation*}
\forall k \in \mathbb{N}\ , \forall i \in \mathbb{N},\ (u^k_i, y^k_i) = (t_{j+i} - t_j, x_{j+i}), \quad \mathrm{where}\ t_j = \xi_k.
\end{equation*}
This definition is such that $u^k_0=0$, and $y^k_i$ is the $i^{th}$ point received
after the $k^{th}$ transmission.

\subsection{LTC algorithm}

Using the notations defined previously, the original LTC algorithm is written as 
follows:
% define x_i (1D), (t, x_i)
\begin{enumerate}
\item Initialization, after $k^{th}$ transmission:
    \begin{itemize}
        \item Wait until first point $(u^k_1, y^k_1)$ is received.
        \item UL $\leftarrow$ $(u^k_1, y^k_1+\epsilon)$
        \item LL $\leftarrow$ $(u^k_1, y^k_1-\epsilon)$
    \end{itemize}
\item Calculate \emph{highLine} as the line connecting $(u^k_0, y^k_0)$ and UL.
\item Calculate \emph{lowLine} as the line connecting $(u^k_0, y^k_0)$ and LL.
\end{enumerate}
...
dimension $n$. The formulation of LTC in~\cite{ltc} relies on the intersection
of \emph{convex cones} in dimension $n+1$.

When $n$ is higher than 1, the intersection of convex cones unwieldy to 
formalize. Instead, we formulate LTC as an intersection test 
between $m$ \emph{balls} of dimension 
$n$. We formulate LTC recursively, as follows.

Let $x_0 \in \mathbb{R}^n$ be the last data point transmitted by the 
algorithm. For convenience, all the subsequent data points will be 
expressed in the orthogonal space with origin $x_0$. Without loss of 
generality, we assume that the data points are sampled uniformly with a 
sampling period of 1 time unit. Let k be the number of data points 
following $x_0$ that have been compressed.

The intersection of convex cones in dimension $n+1$ can be written as follows:
\ldots

Which is equivalent to the following formulation in $\mathbb{R}^n$:
\begin{eqnarray*}
&& \exists \textbf{u} \in \mathbb{R}^{n} \  s.t. \  \forall i \in \llbracket 1, k \rrbracket, \quad
\norm{i\textbf{u} - \textbf{x}_i} \leq \epsilon\\
\Leftrightarrow && \exists \textbf{u} \in \mathbb{R}^{n} \  s.t. \  \forall i \in \llbracket 1, k \rrbracket, \quad
\norm{\textbf{u} - \frac{1}{i}\textbf{x}_i} \leq \frac{\epsilon}{i} \\
\Leftrightarrow && \bigcap_{i=1}^{k} {\mathcal{B}}_i \neq \O,
\end{eqnarray*}
where $\mathcal{B}_i 
\subset \mathbb{R}^n$ is the ball of center $\frac{1}{i}\textbf{x}_i$ 
and radius $\frac{\epsilon}{i}$.
...
%~ Point $k+1$ will be compressed if and only if:
%~ \begin{eqnarray*}
%~ && \bigcap_{i=1}^{k+1} {\mathcal{B}}_i \neq \O \\
%~ \Leftrightarrow && {\mathcal{B}}_{k+1} \cap \bigcap_{i=1}^{k} {\mathcal{B}}_i \neq \O
%~ \end{eqnarray*}
