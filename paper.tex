...
is a rectangular cuboid. So only one bounding box with n-dimansional
...
\begin{enumerate}
    ...
    \item For the each $m_{th}$ data $(u_m^k, y_m^k)$ with diminsion $n$,
    ...
    \item The $\mathcal{B}_{m}$ should be a ractangular cube with center
            $\frac{v_1}{v_j}z_m$ and weight $\frac{v_1}{v_j}\epsilon$.
    \item For each dimension $i$, box upper bound $U_i < \frac{v_1}{v_j}x_{mi}+
            \frac{v_1}{v_j}\epsilon$ OR lower bound $L_i > \frac{v_1}{v_j}x_{mi}-\frac{v_1}{v_j}\epsilon$, return False.
    \item Otherwise, the intersection do not be empty. Updating
            ...
            each dimension $i$ in $\mathcal{BOX}_m$, $U_i = min(U_i, \frac{v_1}{v_j}x_{mi}+
            \frac{v_1}{v_j}\epsilon})$, and $L_i = max(L_i , \frac{v_1}{v_j}x_{mi}-\frac{v_1}{v_j}\epsilon})$. Return True.
\end{enumerate}
