...
Algorithm~\ref{algo:general-ltc}. The implementation only requires 
constant memory and reduced CPU time to update the cuboid with the 
n-ball associated with the new data point at each iteration.

With the Euclidean norm, the intersection test is more complex, as 
mentioned previously. We keep in memory a growing set $S$ of balls and 
the bounding box $B$ of their intersection. Then, when a new point 
arrives, we consider the associated ball $\mathcal{B}_j$ and our 
intersection test works as in Algorithm~\ref{algo:euclidean}. 
\texttt{box} is a function that returns the bounding box of an n-ball. 
\texttt{find\_bisection(S, B)} searches for a point in all the elements 
...
We acquired two 3D accelerometer time series at 200~Hz corresponding to
