...
after that 使用二分法 mid_x=(C_maxX + C_minX)/2, with x=mid_x computer bounding range of y-axis, 并对y轴继续使用 二分法，在z-axis 上计算 C_minZ and C_maxZ, 如果 C_maxZ > C_minZ 说明 在x=mid_x, y=mid_y 这条线上 存在一个或多个点 在intersection amount those circles. so 我们可以确认 确认交集的存在，并且 把点(mid_x, mid_y, mid_z) 保存下来，以便将来更新base point.
